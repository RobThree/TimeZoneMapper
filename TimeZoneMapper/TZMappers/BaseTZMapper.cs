...
public abstract class BaseTZMapper : ITZMapper
{
    ...
    public bool TryMapTZID(string tzid, out TimeZoneInfo timeZoneInfo)
        => _mappings.TryGetValue(tzid, out timeZoneInfo);
    ...
    public TimeZoneInfo[] GetAvailableTimeZones()
        => _mappings.Values.Distinct().ToArray();
}
...
internal static class LinqExtensions
{
    ...
    {
        var ret = new Dictionary<TKey, TElement>(comparer ?? EqualityComparer<TKey>.Default);
...
{
    public static string? GetSafeValue(this XAttribute att, string? defaultValue = null)
        => att == null ? defaultValue : att.Value;
}