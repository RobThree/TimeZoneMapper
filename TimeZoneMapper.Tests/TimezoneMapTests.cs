...
public class TimeZoneMapTests
{
    ...
    public void TryMapTZIDReturnsExpectedValues()
    {
        ...
        Assert.IsTrue(mapper.TryMapTZID("Europe/Amsterdam", out var actual));
        Assert.AreEqual(expected, actual);
    ...
    public void CustomTZMapperThrowsOnNonExistingFile() => _ = new CustomValuesTZMapper("XXX.xml", Encoding.UTF8);

    [TestMethod]
    [ExpectedException(typeof(XmlException), "When invalid XML is specified the CustomTZMapper should throw")]
    public void CustomTZMapperThrowsOnInvalidXML() => _ = new CustomValuesTZMapper("<xxx>");

    [TestMethod]
    [ExpectedException(typeof(InvalidOperationException), "Valid XML, but not valid CLDR data should throw")]
    public void CustomTZMapperThrowsOnInvalidCLDR() => _ = new CustomValuesTZMapper("<foo/>");

    [TestMethod]
    public void CustomTZMapperLoadsEmptyCLDRCorrectly() => _ = new CustomValuesTZMapper("<mapTimezones/>");
    ...
    [TestMethod]
    [ExpectedException(typeof(KeyNotFoundException), "A non-existing TZID should throw")]
    public void DefaultValuesTZMapperThrowsOnNonExistingTZID() => _ = TimeZoneMap.DefaultValuesTZMapper.MapTZID("XXX");
